...
To prevent `run_hypothesis_premise.py` from trying to use a GPU for training, pass the argument `--no_cuda`.

For hypothesis only model run the same command with `run_hypothesis_only.py` file.
